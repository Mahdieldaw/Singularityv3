...
export const ComparisonMatrixContainer: React.FC<ComparisonMatrixContainerProps> = ({ content }) => {
    return (
        <div className="flex flex-col gap-6 p-4 text-white">
                    ...
                    <div key={idx} className="bg-white/5 border border-white/10 rounded-lg p-3 flex flex-col gap-2">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-1">
                                {dim.sources.map(s => (
                                    <span key={s} className="text-[10px] bg-white/10 text-gray-500 px-1.5 py-0.5 rounded">{s}</span>
                                ))}
                            </div>
                        </div>
            ...
            {/* Mini Score Matrix (simplified visualization) */}
                    ...
                    <thead>
                        <tr className="border-b border-gray-700">
                            ...
                            {content.matrix.approaches.map((app, i) => (
                                <th key={i} className="p-2 font-medium text-gray-300">{app}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                                    ...
                                    <td key={colIdx} className="p-2">
                                        <div className="flex items-center gap-2">
                                            <div className="w-full bg-gray-800 rounded-full h-1.5 max-w-[60px]">
                                                <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${score * 10}%` }} />
                                            </div>
